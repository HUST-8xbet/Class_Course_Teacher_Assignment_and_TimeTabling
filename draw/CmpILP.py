...
OUTPUT_FILE = "Figure/Exact_Solvers_Comparison_ms.png"
...
def plot_exact_solvers_scaling():
    ...
    plt.title(f"SO SÁNH TỐC ĐỘ THỰC THI CỦA CÁC BỘ GIẢI CHÍNH XÁC", 
              fontsize=14, fontweight='bold', pad=15)
    
    plt.xlabel("Số lớp (N)", fontsize=12)
    # Đã sửa lại tiêu đề trục Y thành Mili-giây
    plt.ylabel("Thời gian chạy trung bình (ms)", fontsize=12) 
    
    plt.yscale('log')
    
    # CỐ ĐỊNH TRỤC X: Bắt đầu từ trước N nhỏ nhất 1 chút, kết thúc chính xác ở 310 (dư 10 để thoáng lề)
    plt.xlim(min(sorted_N) - 10, MAX_N_LIMIT + 10)
    ...
    plt.close()
    print(f"✅ Đã vẽ xong! Biểu đồ so sánh 3 bộ giải chính xác được lưu tại: {OUTPUT_FILE}")
